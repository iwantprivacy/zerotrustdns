...
export const FETCH_MAX_SOURCES = 32;
export const FETCH_MAX_TOTAL_BYTES = 200 * 1024 * 1024;
...
async function fetchAll(urls, sourceKind, options = {}) {
  const maxSources = options.maxSources ?? FETCH_MAX_SOURCES;
  const maxTotalBytes = options.maxTotalBytes ?? FETCH_MAX_TOTAL_BYTES;
  const budget = options.budget ?? { sourceCount: 0, totalBytes: 0 };
  ...
  for (const [index, url] of urls.entries()) {
    ...
    try {
      ...
      const text = await fetchOne(url, {
        ...options,
        maxBytes: Math.min(options.maxBytes ?? FETCH_MAX_BYTES, remainingBytes),
      });
...
function domainsFromLine(line, isAllowlist = false) {
  ...
  if (isIP(tokens[0])) {
    const hostTokens = tokens.slice(1).join(" ").split("#", 1)[0].trim().split(/\s+/).filter(Boolean);
    return hostTokens.map((token) => normalizeDomain(token)).filter(Boolean);
  }
...
export function parseDomains(blocklistRaw, allowlistRaw, limit = LIST_ITEM_LIMIT) {
  ...
  Object.defineProperties(result, {
    candidateCount: { value: ordered.length, enumerable: false },
