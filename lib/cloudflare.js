...
const LIST_NAME_RE = /^zerotrustdns List - Chunk ([1-9]\d*)$/;
const RULE_NAME = "zerotrustdns Filter Lists";
const RULE_DESCRIPTION = "Managed by zerotrustdns. Do not rename this rule.";
const LIST_DESCRIPTION = "Managed by zerotrustdns. Do not rename this list.";

function hasForeignDescription(resource, expected) {
  const description = resource?.description;
  if (description == null) return false;
  if (typeof description !== "string") return true;
  return description.trim() !== "" && description !== expected;
}
...
export function isManagedList(list) {
  return (
    list?.type === "DOMAIN" &&
    LIST_NAME_RE.test(list?.name ?? "") &&
    !hasForeignDescription(list, LIST_DESCRIPTION)
  );
}

export function isManagedRule(rule) {
  return rule?.name === RULE_NAME && !hasForeignDescription(rule, RULE_DESCRIPTION);
}
...
async function cfFetch(path, options = {}) {
  ...
  for (let attempt = 1; attempt <= CF_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (res.status === 204) return { success: true, result: null };
      if (res.ok) {
      ...
      try {
        const payload = await res.json();
        providerDetails = (payload?.errors ?? [])
      ...
      const apiError = new Error(
        `Cloudflare API error ${res.status} on ${path}${providerDetails ? `: ${providerDetails}` : ""}`
      );
      apiError.status = res.status;
      apiError.retryAfterMs = retryAfterMs(res);
      apiError.providerDetails = providerDetails;
      throw apiError;
    } catch (err) {
      ...
      if (!canRetry) {
        ...
        if (status) {
          const detailNote = err?.providerDetails ? `: ${err.providerDetails}` : "";
          ...
          const finalError = new Error(`Cloudflare API error ${status} on ${path}${detailNote}${mutationNote}${attemptNote}`);
          finalError.status = status;
          finalError.providerDetails = err?.providerDetails ?? "";
          finalError.ambiguousMutation = retryable && !retrySafe;
          throw finalError;
        }
        if (err?.name === "AbortError") {
          const timeoutError = new Error(`Cloudflare API timeout on ${path}`);
          timeoutError.ambiguousMutation = !retrySafe;
          throw timeoutError;
        }
        const networkError = new Error(`Cloudflare API network error on ${path}: ${err.message}`);
        networkError.ambiguousMutation = !retrySafe;
        throw networkError;
      }
...
const createList = (name, items) =>
  cfFetch("/lists", {
    method: "POST",
    body: JSON.stringify({ name, description: LIST_DESCRIPTION, type: "DOMAIN", items }),
  });
...
function buildListPatch(current, target, description) {
  const targetCounts = new Map();
  for (const value of target) targetCounts.set(value, (targetCounts.get(value) ?? 0) + 1);
  const remove = [];
  for (const value of current) {
    const remaining = targetCounts.get(value) ?? 0;
    if (remaining > 0) targetCounts.set(value, remaining - 1);
    else remove.push(value);
  }
  ...
  for (const value of target) {
    const remaining = currentCounts.get(value) ?? 0;
    if (remaining > 0) currentCounts.set(value, remaining - 1);
    else append.push({ value, description });
  }
...
function isVerifiedRule(rule, traffic) {
  ...
  const settingsValid =
    settings == null ||
    ((settings.block_page_enabled == null || settings.block_page_enabled === false) &&
...
async function rollbackListMutations(existingLists, originalItemsByList, patches, createdLists) {
  ...
  for (const list of existingLists) {
    ...
    try {
      ...
      await patchList(list.id, buildListPatch(current, original, "rollback"));
...
export async function syncLists(domains) {
  ...
  const namedLists = (allLists || []).filter(
    (list) => list?.type === "DOMAIN" && LIST_NAME_RE.test(list?.name ?? "")
  );
  const foreignNamedLists = namedLists.filter(
    (list) => hasForeignDescription(list, LIST_DESCRIPTION)
  );
  if (foreignNamedLists.length) {
    throw new Error(`Cloudflare list ownership collision: ${foreignNamedLists.map(({ name }) => name).join(", ")}`);
  }
  const nameCounts = new Map();
  for (const list of namedLists) nameCounts.set(list.name, (nameCounts.get(list.name) ?? 0) + 1);
  const duplicateNames = [...nameCounts.entries()].filter(([, count]) => count > 1).map(([name]) => name);
  if (duplicateNames.length) {
    throw new Error(`Duplicate managed Cloudflare list names: ${duplicateNames.join(", ")}`);
  }
  const existingLists = sortManagedLists(namedLists);
  ...
  let obsoleteLists = existingLists.filter((list) => finalCountByList.get(list.id) === 0);
  for (const list of obsoleteLists) patches.delete(list.id);

  let createNames = nextChunkNames(existingLists, Math.ceil(toAdd.length / LIST_CHUNK_SIZE));
  let createSpecs = createNames.map((name, index) => ({
    name,
    items: appendItems(toAdd.slice(index * LIST_CHUNK_SIZE, (index + 1) * LIST_CHUNK_SIZE)),
  }));

  const unmanagedListCount = allLists.length - existingLists.length;
  let finalManagedListCount =
    [...finalCountByList.values()].filter((count) => count > 0).length + createSpecs.length;
  let projectedAccountListCount = unmanagedListCount + finalManagedListCount;

  // If incremental cleanup still leaves too many managed lists, repack the
  // desired domains into contiguous chunks before checking the account quota.
  if (projectedAccountListCount > LIST_ACCOUNT_LIMIT) {
    const requiredManagedListCount = Math.ceil(uniqueDomains.length / LIST_CHUNK_SIZE);
    const activeExistingLists = existingLists.slice(0, requiredManagedListCount);
    const compactedPatches = new Map();

    for (const [index, list] of activeExistingLists.entries()) {
      const targetItems = uniqueDomains.slice(index * LIST_CHUNK_SIZE, (index + 1) * LIST_CHUNK_SIZE);
      const currentItems = itemsByList.get(list.id) ?? [];
      if (!sameValues(currentItems, targetItems)) {
        compactedPatches.set(list.id, buildListPatch(currentItems, targetItems, now));
      }
    }

    patches = compactedPatches;
    obsoleteLists = existingLists.slice(requiredManagedListCount);
    finalCountByList = new Map(
      existingLists.map((list, index) => [
        list.id,
        index < requiredManagedListCount
          ? Math.min(LIST_CHUNK_SIZE, uniqueDomains.length - index * LIST_CHUNK_SIZE)
          : 0,
      ])
    );

    createNames = nextChunkNames(existingLists, Math.max(0, requiredManagedListCount - existingLists.length));
    createSpecs = createNames.map((name, index) => {
      const chunkIndex = existingLists.length + index;
      return {
        name,
        items: appendItems(uniqueDomains.slice(chunkIndex * LIST_CHUNK_SIZE, (chunkIndex + 1) * LIST_CHUNK_SIZE)),
      };
    });

    finalManagedListCount = requiredManagedListCount;
    projectedAccountListCount = unmanagedListCount + finalManagedListCount;
    if (projectedAccountListCount <= LIST_ACCOUNT_LIMIT) {
      console.log(`  Compacting managed domains into ${requiredManagedListCount} list(s)...`);
    }
  }

  if (projectedAccountListCount > LIST_ACCOUNT_LIMIT) {
    throw new Error(
      `Cloudflare list quota would be exceeded: ${projectedAccountListCount} projected lists > ${LIST_ACCOUNT_LIMIT} allowed`
    );
  }

  const totalRemovals = [...patches.values()].reduce((sum, patch) => sum + (patch.remove?.length ?? 0), 0);
  const createdLists = [];
  ...
  try {
    for (const spec of createSpecs) {
      ...
      if (typeof result?.id === "string" && result.id) {
        createdLists.push({
          ...
          description: result.description ?? LIST_DESCRIPTION,
      ...
      if (
        ...
        result.type !== "DOMAIN" ||
        hasForeignDescription(result, LIST_DESCRIPTION)
      ) {
        throw new Error(`Cloudflare returned an invalid identity for created list ${spec.name}`);
      }
    ...
    // Read back every modified or created managed list before allowing the rule update.
...
export async function upsertRule(lists) {
  ...
  let ruleWriteCommitted = false;
  try {
    const { result: existingRules } = await getRules();
    const sameNameRules = existingRules.filter((rule) => rule?.name === RULE_NAME);
    const foreignSameNameRules = sameNameRules.filter((rule) => hasForeignDescription(rule, RULE_DESCRIPTION));
    if (foreignSameNameRules.length) {
      throw new Error("Cloudflare rule ownership collision: a different rule already uses the managed name");
    }
    const managedRules = sameNameRules.sort((a, b) => String(a.id).localeCompare(String(b.id)));
    ...
    error.ruleMutationAmbiguous =
      ruleWriteCommitted || error.ambiguousMutation === true;
    throw error;
  }
}
