...
export const LIST_DESCRIPTION = "Managed by zerotrustdns. Do not rename this list.";
...
export async function syncLists(domains) {
  ...
  const existingLists = sortManagedLists(namedLists.filter(isManagedList));
...
export async function upsertRule(lists) {
  ...
  let mutationState = "not_attempted";
  try {
    ...
    const managedRules = sameNameRules.filter(isManagedRule).sort((a, b) => String(a.id).localeCompare(String(b.id)));
    const [canonical, ...duplicates] = managedRules;

    if (canonical) {
      mutationAttempted = true;
      mutationState = "attempting";
    ...
    error.ruleMutationAmbiguous =
      mutationAttempted && (mutationState === "committed" || error.ambiguousMutation === true);
    throw error;
  }
}
